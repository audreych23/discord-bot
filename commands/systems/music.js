...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (!VoiceChannel) {return interaction.reply({ content: 'You aren\'t in a voice channel. Join one to be able to play music!', ephemeral: true });}
    ...
    try {
      ...
      case 'play':
      {
        ...
        if (player.queue.totalSize > 1) {enqueueEmbed.addField('Position in queue', `${player.queue.size - 0}`);}
